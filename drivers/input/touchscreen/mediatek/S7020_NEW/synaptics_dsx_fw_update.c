// add some log and fix cts issue
/*
...
static char *G630_junda_panel = "G630HW-005";
static char *G630_ofilm_panel = "G630HW-000";
static char *G630_ofilm_panel_new ="G630MV-000";
static char *H30_ofilm_panel ="HR3CHW-000";
static char *H30_truly_panel ="HR3CHW-002";
#define G630_FW_IMAGE_OFILM_NEW_NAME "G630_startup_fw_update_ofilm_new.img"
#define G630_FW_IMAGE_JUNDA_NAME "G630_startup_fw_update_junda.img"
#define G630_FW_IMAGE_OFILM_NAME "G630_startup_fw_update_ofilm.img"
#define H30_FW_IMAGE_OFILM_NAME "H30_startup_fw_update_ofilm.img"
#define H30_FW_IMAGE_TRULY_NAME "H30_startup_fw_update_truly.img"
...
static int fwu_read_f01_device_status(struct f01_device_status *status)
{
	...
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to read F01 device status\n",
				__func__,__LINE__);
		return retval;
	}
...
static int fwu_read_f34_queries(void)
{
	...
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to read flash properties\n",
				__func__,__LINE__);
		return retval;
	}
	...
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to read block size info\n",
				__func__,__LINE__);
		return retval;
	}
	...
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to read block count info\n",
				__func__,__LINE__);
		return retval;
	}
...
static int fwu_scan_pdt(void)
{
	...
	if (!f01found || !f34found) {
		tp_log_err("%s(line %d): "
				"Failed to find both F01 and F34\n",
				__func__,__LINE__);
		return -EINVAL;
	}
		...
		unsigned char command)
{
	...
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to write to block number registers\n",
				__func__,__LINE__);
		return retval;
	}
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		if (retval < 0) {
			tp_log_err("%s(line %d): "
				"Failed to write block data (block %d)\n",
		...
		if (retval < 0) {
			tp_log_err("%s(line %d): "
				"Failed to write command for block %d\n",
				__func__,__LINE__, block_num);
			return retval;
		}
...
static int fwu_enter_flash_prog(void)
{
	...
	tp_log_info("%s(line %d):fwu_write_bootloader_id ,retval=%d ",__func__,__LINE__,retval);
	if (retval < 0)
		return retval;

	retval = fwu_write_f34_command(CMD_ENABLE_FLASH_PROG);
	tp_log_info("%s(line %d):fwu_write_f34_command ,retval=%d",__func__,__LINE__,retval);
	if (retval < 0)
		return retval;

	retval = fwu_wait_for_idle(ENABLE_WAIT_MS);
	tp_log_info("%s(line %d):fwu_wait_for_idle ,retval=%d",__func__,__LINE__,retval);
	if (retval < 0)
		return retval;
	...
	retval = fwu_scan_pdt();
	tp_log_info("%s(line %d):fwu_scan_pdt ,retval=%d",__func__,__LINE__,retval);
	if (retval < 0)
		return retval;

	retval = fwu_read_f01_device_status(&f01_device_status);
	tp_log_info("%s(line %d):fwu_read_f01_device_status ,retval=%d",__func__,__LINE__,retval);
	if (retval < 0)
		return retval;
	...
	tp_log_info("%s(line %d):fwu_read_f34_queries ,retval=%d",__func__,__LINE__,retval);
	if (retval < 0)
		return retval;
	...
	tp_log_info("%s(line %d): read,retval=%d",__func__,__LINE__,retval);
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to read F01 device control\n",
				__func__,__LINE__);
		return retval;
	}
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			f01_device_control.data,
			sizeof(f01_device_control.data));
	tp_log_info("%s(line %d): write,retval=%d",__func__,__LINE__,retval);
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to write F01 device control\n",
				__func__,__LINE__);
		return retval;
	}
...
static int fwu_do_reflash(void)
{
	...
	if (fwu->firmware_data) {
		...
		if (retval < 0)
			return retval;
		tp_log_info("%s(line %d): Firmware programmed\n", __func__,__LINE__);
	}
	...
	if (fwu->config_data) {
		...
		if (retval < 0)
			return retval;
		tp_log_info("%s(line %d): Configuration programmed\n", __func__,__LINE__);
	}
...
write_config:
	...
	tp_log_info("%s(line %d): Config written\n", __func__,__LINE__);
...
static int fwu_start_write_config(void)
{
	...
	tp_log_info("%s(line %d): Start of write config process\n",__func__,__LINE__);
	...
	tp_log_info("%s(line %d): End of write config process\n",__func__,__LINE__);
...
static int fwu_do_read_config(void)
{
	...
	tp_log_info("%s(line %d): Entered flash prog mode\n",__func__,__LINE__);
	...
	for (block_num = 0; block_num < block_count; block_num++) {
		...
		if (retval < 0) {
			tp_log_err("%s(line %d): "
				"Failed to write read config command\n",
				__func__,__LINE__);
			goto exit;
		}
		...
		if (retval < 0) {
			tp_log_err("%s(line %d): "
				"Failed to wait for idle status\n",
				__func__,__LINE__);
			goto exit;
		}
		...
		if (retval < 0) {
			tp_log_err("%s(line %d): "
				"Failed to read block data (block %d)\n",
				__func__,__LINE__,block_num);
			goto exit;
		}
...
static int fwu_do_lockdown(void)
{
	...
	if ((fwu->flash_properties & UNLOCKED) == 0) {
		tp_log_info("%s(line %d): Device already locked down\n",__func__,__LINE__);
		return retval;
	}
	...
	tp_log_info("%s(line %d): Lockdown programmed\n",__func__,__LINE__);
...
static char * get_cof_module_name(u8 module_id)
{
    ...
    switch(module_id)
    {
        case TP_COF_ID_OFILM:
			 return "ofilm";
        case TP_COF_ID_JUNDA:
			 return "junda";
        case TP_COF_ID_TRULY:
			 return "truly";
        default:
			 return "unknow";
    }
...
static void synaptics_set_appinfo(void)
{
	...
	sprintf(touch_info,"synaptics_dsx_%s.%x",get_cof_module_name(config_id[2]),config_id[3]);
...
char *get_module_name(void)
{
	...
	return(get_cof_module_name(config_id[2]));
exit:
	return(NULL);
}
static int fwu_start_reflash(void)
{
	...
	tp_log_info("%s(line %d): Start of reflash process\n", __func__,__LINE__);

	if (fwu->ext_data_source) {
		fw_image = fwu->ext_data_source;
	} else {
		if (strcmp(fwu->product_id,G630_ofilm_panel)==0){
			strncpy(fwu->image_name, G630_FW_IMAGE_OFILM_NAME, MAX_IMAGE_NAME_LEN);
			}
		else if (strcmp(fwu->product_id,G630_junda_panel)==0){
			strncpy(fwu->image_name, G630_FW_IMAGE_JUNDA_NAME, MAX_IMAGE_NAME_LEN);
			}
		else if (strcmp(fwu->product_id,G630_ofilm_panel_new)==0){
			strncpy(fwu->image_name, G630_FW_IMAGE_OFILM_NEW_NAME, MAX_IMAGE_NAME_LEN);
			}
		else if (strcmp(fwu->product_id,H30_ofilm_panel)==0){
			strncpy(fwu->image_name, H30_FW_IMAGE_OFILM_NAME, MAX_IMAGE_NAME_LEN);
			}
		else if (strcmp(fwu->product_id,H30_truly_panel)==0){
			strncpy(fwu->image_name, H30_FW_IMAGE_TRULY_NAME, MAX_IMAGE_NAME_LEN);
			}
...
exit:
	...
	tp_log_info("%s(line %d):End of reflash process, retval = %d\n",__func__,__LINE__,retval);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < fwu->config_size) {
		tp_log_err("%s(line %d): "
				"Not enough space (%d bytes) in buffer\n",
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!fwu->ext_data_source) {
		tp_log_err("%s(line %d): "
				"Failed to alloc mem for image data\n",
				__func__,__LINE__);
		return -ENOMEM;
	}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!fwu) {
		tp_log_err("%s(line %d): Failed to alloc mem for fwu\n",__func__,__LINE__);
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!fwu->fn_ptr) {
		tp_log_err("%s(line %d): Failed to alloc mem for fn_ptr\n",__func__,__LINE__);
		retval = -ENOMEM;
		goto exit_free_fwu;
	}
	...
	if (!fwu->image_name) {
		tp_log_err("%s(line %d): Failed to alloc mem for image name\n",__func__,__LINE__);
		retval = -ENOMEM;
	...
	if (retval < 0) {
		tp_log_info("%s(line %d): Failed to read PDT properties, assuming 0x00\n",__func__,__LINE__);
	} else if (pdt_props.has_bsr) {
		tp_log_err("%s(line %d): Reflash for LTS not currently supported\n",__func__,__LINE__);
		retval = -ENODEV;
		goto exit_free_mem;
	}
	...
	tp_log_info("%s(line %d): F01 product info: 0x%04x 0x%04x\n",
			__func__,__LINE__,fwu->productinfo1, fwu->productinfo2);

	tp_log_info("%s(line %d): F01 product ID: %s\n",
	...
	retval = sysfs_create_bin_file(&rmi4_data->input_dev->dev.kobj,
			&dev_attr_data);
	if (retval < 0) {
		tp_log_err("%s(line %d): Failed to create sysfs bin file\n",__func__,__LINE__);
		goto exit_free_mem;
	}
	...
	if(get_boot_mode() == NORMAL_BOOT){
		...
		{
			tp_log_info("%s(line %d): don't auto update FW in other boot mode.\n",__func__,__LINE__);
		}
#endif
