...
#define DELAY_S7300_RESET_READY 250
...
// for MTK
//exlusive isr report & suspend free fingers	
DEFINE_MUTEX( isr_suspend_mutex );
...
#endif
static u8 boot_mode;
extern unsigned char cyttsp_device_check_ok;
// for DMA accessing
...
uint16_t  synaptics_gesture_key_event[GESTURE_MAX] = {
		KEY_F1,
		KEY_F2,
		KEY_F3,
		KEY_F4,
...
uint16_t synapitc_gesture_direction_dectected[GESTURE_MAX] ={
		0,
		1,
		2,
		3,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to issue reset command, error = %d\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
		tp_log_err("%s(line %d): "
				"Failed to read device status, error = %d\n",
				__func__,__LINE__, retval);
		return retval;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, sizeof(easywake_position)*2, "%08x%08x",easywake_position[0], \
		easywake_position[1]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ssize_t ret;
	mutex_lock(&rmi4_data->rmi4_sysfs_mutex);
	ret = snprintf(buf, sizeof(rmi4_data->easy_wakeup_gesture)*2, "0x%04x\n",rmi4_data->easy_wakeup_gesture+rmi4_data->easy_wakeup_support*128 );
	mutex_unlock(&rmi4_data->rmi4_sysfs_mutex);
	return ret;
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	mutex_lock(&rmi4_data->rmi4_sysfs_mutex);
	if (1)
		rmi4_data->easy_wakeup_gesture = (u32)GESTURE_FROM_APP(value&0x007f);
	else
		ret = -ENODEV;
	mutex_unlock(&rmi4_data->rmi4_sysfs_mutex);
	
	if(rmi4_data->touch_stopped == true){
			 if( rmi4_data->easy_wakeup_gesture != 0 ){
					ret = synaptics_rmi4_i2c_read(rmi4_data,
								rmi4_data->f01_ctrl_base_addr,
								&device_ctrl,
								sizeof(device_ctrl));
					if (ret < 0) {
							tp_log_err("%s: Failed to wake from sleep mode\n", __func__);
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	if (!cdev){
		tp_log_info("%s(line %d): device is null\n",__func__,__LINE__);
		return 0;
	}
...
static struct kobj_attribute easy_wakeup_position = {
	.attr = {.name = "easy_wakeup_position", .mode = (0664)},
...
static ssize_t hw_synaptics_easy_wakeup_control_store(struct kobject *dev,
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct device *cdev = core_dev_ptr;
	unsigned long value;
...
static struct kobj_attribute easy_wakeup_control = {
	...
	.show = NULL,
	.store = hw_synaptics_easy_wakeup_control_store,
};
#endif
	...
	if (page != rmi4_data->current_page) {
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval != PAGE_SELECT_LEN) {
				tp_log_err("%s(line %d): I2C retry %d\n",	__func__,__LINE__, retry + 1);
				msleep(20);
			} else {
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	mutex_lock(&(rmi4_data->rmi4_io_ctrl_mutex));
        ...
        if(!gprDMABuf_va){
			tp_log_err("%s(line %d):Allocate DMA I2C Buffer failed!\n",__func__,__LINE__);
        }
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		tp_log_err("%s(line %d):I2C read over retry linit\n",__func__,__LINE__);
		retval = -EIO;
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		tp_log_err("%s(line %d):I2C write over retry limit\n",__func__,__LINE__);
		retval = -EIO;
	}
...
static struct kobject* tp_get_touch_screen_obj(void)
{
	...
	{
		touch_screen_kobject = kobject_create_and_add("touch_screen_easy_wakeup", NULL);
		if (!touch_screen_kobject)
		{
	...
	else
	{
		...
		return kobject_get(touch_screen_kobject);		
	}
	
	return touch_screen_kobject;
}
		...
		unsigned int reprot_gesture_point_num,unsigned int reprot_gesture_key_value)
{
	...
	tp_log_info("%s:reprot_gesture_point_num =%0x \n)",__func__,reprot_gesture_point_num);
	if (reprot_gesture_point_num != 0)
	{
		...
		for(i = 0;i < reprot_gesture_point_num;i ++)
		{
			...
			*/
			...
			x = x * LCM_WIDTH / rmi4_data->sensor_max_x;
			y = y * RATIO / rmi4_data->sensor_max_y;
			if( (x > LCM_WIDTH ) ||( x <0)){
				tp_log_err("%s:x position over the size ,x = %d ",__func__,x );
			}
			if( (y > RATIO) ||( y <0)){
				tp_log_err("%s:x position over the size ,y = %d ",__func__,y );
			}			
		...
		struct synaptics_rmi4_fn *fhandler)
{
   ...
   unsigned int reprot_gesture_point_num = 0;
	...
	tp_log_debug("%s:f11_data_base_addr = %d,device_data = %d",
							__func__, rmi4_data->f11_data_base_addr,device_data);

	switch(device_data)
	{
		case DOUBLE_CLICK_WAKEUP:
			if(IS_APP_ENABLE_GESTURE(GESTURE_DOUBLE_CLICK)&rmi4_data->easy_wakeup_gesture)
			{
		...
		case LINEAR_SLIDE_DETECTED:
			tp_log_debug("%s:@@@LINEAR_SLIDE_DETECTED detected!@@@ \n",__func__);
			retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			if (retval < 0)
			{
				tp_log_err("%s(line %d):LINEAR_SLIDE_DETECTED error!retval = %d\n",__func__,__LINE__,retval);
				return 0;
			}
			tp_log_debug("%s:@@@f51_custom_data0 = %d@@@",__func__,f51_custom_data0);
			switch(f51_custom_data0)
			{
				case LINEAR_SLIDE_LEFT_TO_RIGHT:
					if(IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_L2R)&rmi4_data->easy_wakeup_gesture)
					{
				...
				case LINEAR_SLIDE_RIGHT_TO_LEFT:
					if(IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_R2L)&rmi4_data->easy_wakeup_gesture)
					{
				...
				case LINEAR_SLIDE_TOP_TO_BOTTOM:
					if(IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_T2B)&rmi4_data->easy_wakeup_gesture)
					{
				...
				case LINEAR_SLIDE_BOTTOM_TO_TOP:
					if(IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_B2T)&rmi4_data->easy_wakeup_gesture)
					{
				...
				case LINEAR_SLIDE_TOP_TO_BOTTOM2:
					if(IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_T2B2)&rmi4_data->easy_wakeup_gesture)
					{
					...
					break;
				default:
					tp_log_debug("%s:@@@unknow LINEAR!f51_custom_data0 = %d@@@",__func__,f51_custom_data0);
					return 0;
			}
			break;
		case CIRCLE_SLIDE_DETECTED:
			if(IS_APP_ENABLE_GESTURE(GESTURE_CIRCLE_SLIDE)&rmi4_data->easy_wakeup_gesture)
			{
				tp_log_debug("%s:@@@CIRCLE_SLIDE_DETECTED detected!@@@",__func__);
		...
		case SPECIFIC_LETTER_DETECTED:
			tp_log_debug("%s:@@@SPECIFIC_LETTER_DETECTED detected!@@@",__func__);
			retval = synaptics_rmi4_i2c_read(rmi4_data,
					rmi4_data->f11_data_base_addr+F11_2D_DATA39_OFFSET,
					&f11_2d_data39[0],
					sizeof(f11_2d_data39));

			tp_log_debug("%s:###read### f11_2d_data39[8] = 0x%x  ",__func__,f11_2d_data39[8]);

			switch(f11_2d_data39[8])
			{
				case SPECIFIC_LETTER_c:
					if(IS_APP_ENABLE_GESTURE(GESTURE_LETTER_c)&rmi4_data->easy_wakeup_gesture)
					{
				...
				case SPECIFIC_LETTER_e:
					if(IS_APP_ENABLE_GESTURE(GESTURE_LETTER_e)&rmi4_data->easy_wakeup_gesture)
					{
				...
				case SPECIFIC_LETTER_m:
					if(IS_APP_ENABLE_GESTURE(GESTURE_LETTER_m)&rmi4_data->easy_wakeup_gesture)
					{
				...
				case SPECIFIC_LETTER_w:
					if(IS_APP_ENABLE_GESTURE(GESTURE_LETTER_w)&rmi4_data->easy_wakeup_gesture)
					{
					...
					break;
				default:
					tp_log_debug("%s:@@@unknow letter!f11_2d_data39[8] = %d@@@",__func__,f11_2d_data39[8]);
					return 0;
			}
			break;
		default:
	...
	{			
		...
		if(1==off_motion_on){
			...
			retval = easy_wakeup_gesture_report_locus(rmi4_data,reprot_gesture_point_num,reprot_gesture_key_value);
             tp_log_debug("Gesture is  reported \n");
			if (retval < 0){
			mutex_unlock(&wrong_touch_lock);
				return 0;
			}
		...
		tp_log_debug("Gesture is  not reported because framwork \n");
		}
		mutex_unlock(&wrong_touch_lock);
	}
	return 0;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (retval < 0)
	{
		tp_log_err("%s : synaptics_rmi4_i2c_read error : %d",__func__,retval);
		return 0;
	}
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			raw_x = (data[0] << 4) | (data[2] & MASK_4BIT);
			raw_y = (data[1] << 4) | ((data[2] >> 4) & MASK_4BIT);
			wx = (data[3] & MASK_4BIT);
			wy = (data[3] >> 4) & MASK_4BIT;

			x = raw_x * LCM_WIDTH/rmi4_data->sensor_max_x;
			y = raw_y* RATIO / rmi4_data->sensor_max_y;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (extra_data->data15_size) {
		...
		tp_log_debug("%s(line %d):Number of fingers to process = %d\n",__func__,__LINE__,fingers_to_process);
	}
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		//	input_report_key(rmi4_data->input_dev,
			...
			tp_log_debug("%s(line %d): Finger %d:\n"
					"status = 0x%02x\n"
					"x = %d\n"
					"y = %d\n"
					"wx = %d\n"
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to read button data registers\n",__func__,__LINE__);
		return;
	}
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		tp_log_debug("%s(line %d):Button %d (code %d) ->%d\n",__func__,__LINE__,button,
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	tp_log_debug("%s(line %d): Function %02x reporting\n",
			__func__, __LINE__,fhandler->fn_number);

	switch (fhandler->fn_number) {
	case SYNAPTICS_RMI4_F11:
		//exlusive isr report & suspend free fingers
		mutex_lock(&isr_suspend_mutex);
		touch_count_2d = synaptics_rmi4_f11_abs_report(rmi4_data,
				fhandler);
		mutex_unlock(&isr_suspend_mutex);
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to read interrupt status\n",__func__,__LINE__);
		return;
	}

	status.data[0] = data[0];
	if (status.unconfigured && !status.flash_prog) {
		tp_log_debug("%s(line %d):spontaneous reset detected\n",__func__,__LINE__);
		retval = synaptics_rmi4_reinit_device(rmi4_data);
		if (retval < 0) {
			tp_log_err("%s(line %d):Failed to reinit device\n",__func__,__LINE__);
		}
		return;
	}
...
static int touch_event_handler(void *data)
{
	...
	unsigned char int_status[MAX_INTR_REGISTERS];
	int retval =0;
	...
	do{
		...
		//when touch_stopped is true, clear interrupts again
		if (!rmi4_data->touch_stopped)
			...
			{
				retval = synaptics_rmi4_i2c_read(rmi4_data,rmi4_data->f01_data_base_addr + 1,
												int_status,rmi4_data->num_of_intr_regs);
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (retval < 0)
		{
			tp_log_err("%s(line %d):synaptics_rmi4_i2c_read error\n",__func__,__LINE__);
			return retval;
		}
		// set up irq
		if (!rmi4_data->irq_enabled) {
			...
			mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINTF_TRIGGER_LOW, tpd_eint_handler, 0);
			}
		mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
		...
		unsigned short ctrl28)
{
	...
	if (retval < 0)
	{
		tp_log_err("%s(line %d):synaptics_rmi4_i2c_write error : %d\n",__func__,__LINE__,retval);
	}
	return retval;
}
		...
		unsigned int intr_count)
{
	...
	rmi4_data->sensor_max_y = rmi4_data->sensor_max_y * TPD_DISPLAY_HEIGH_RATIO / TPD_TOUCH_HEIGH_RATIO;
#endif
	tp_log_debug("%s(line %d):Function %02x max x = %d max y = %d\n",
			__func__,__LINE__, fhandler->fn_number,
			rmi4_data->sensor_max_x,
			rmi4_data->sensor_max_y);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!f1a) {
		tp_log_err("%s(line %d):Failed to alloc mem for function handle\n",
			__func__,__LINE__);
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to read query registers\n",
			__func__,__LINE__);
		return retval;
	}
	...
	if (!f1a->button_control.txrx_map) {
		tp_log_err("%s(line %d):Failed to alloc mem for tx rx mapping\n",
			__func__,__LINE__);
		return -ENOMEM;
	}
	...
	if (!f1a->button_data_buffer) {
		tp_log_err("%s(line %d):Failed to alloc mem for data buffer\n",
			__func__,__LINE__);
		return -ENOMEM;
	}
	...
	if (!f1a->button_map) {
		tp_log_err("%s(line %d):Failed to alloc mem for button map\n",
			__func__,__LINE__);
		return -ENOMEM;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (f1a->button_query.has_tx_rx_map) {
		...
		if (retval < 0) {
			tp_log_err("%s(line %d):Failed to read tx rx mapping\n",
			__func__,__LINE__);
			return retval;
		}
		...
		bool *was_in_bl_mode)
{
	...
	if (status.flash_prog == 1) {
		...
		pr_notice("%s: In flash prog mode, status = 0x%02x\n",
				__func__,
				status.status_code);
		tp_log_info("%s(line %d):In flash prog mode, status = 0x%02x\n",__func__,__LINE__,status.status_code);
	} else {
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to set configured\n",__func__,__LINE__);		
		return;
	}
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			retval = synaptics_rmi4_i2c_read(rmi4_data,
					...
					sizeof(rmi_fd));
			if (retval < 0){
				tp_log_err("%s(line %d):synaptics_rmi4_i2c_read error : %d\n",__func__,__LINE__,retval);
				return retval;
			}
			...
			if (rmi_fd.fn_number == 0) {
				tp_log_info("%s(line %d):Reached end of PDT\n",__func__,__LINE__);
				break;
			}

			tp_log_info("%s(line %d):F%02x found (page %d)\n",__func__,__LINE__,rmi_fd.fn_number,page_number);
			...
			case SYNAPTICS_RMI4_F01:
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
						&rmi_fd, page_number);
				if (retval < 0) {
					tp_log_err("%s(line %d):Failed to alloc for F%d\n",__func__,__LINE__,rmi_fd.fn_number);
					return retval;
				}
				...
				if (retval < 0)
				{
					tp_log_info("%s(line %d):synaptics_rmi4_f01_init:%d\n",__func__,__LINE__,retval);
					return retval;
				}
				retval = synaptics_rmi4_check_status(rmi4_data,
						&was_in_bl_mode);
				if (retval < 0) {
					tp_log_err("%s(line %d):Failed to check statusn",__func__,__LINE__);
			...
			case SYNAPTICS_RMI4_F11:
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
						&rmi_fd, page_number);
				if (retval < 0) {
					tp_log_err("%s(line %d): Failed to alloc for F%d",__func__,__LINE__,rmi_fd.fn_number);
					return retval;
				}
				...
				if (retval < 0)
				{
					tp_log_info("%s(line %d):synaptics_rmi4_f11_init:%d\n",__func__,__LINE__,retval);
					return retval;
				}
				break;
			case SYNAPTICS_RMI4_F12:
				...
				if (retval < 0)
				{
					tp_log_info("%s(line %d):synaptics_rmi4_f12_init:%d\n",__func__,__LINE__,retval);
					return retval;
				}
				break;
			case SYNAPTICS_RMI4_F1A:
				...
				retval = synaptics_rmi4_f1a_init(rmi4_data,
						fhandler, &rmi_fd, intr_count);
				if (retval < 0) {
					tp_log_info("%s(line %d):synaptics_rmi4_f1a_init:%d\n",__func__,__LINE__,retval);
			...
			case SYNAPTICS_RMI4_F51:
				...
				if (retval < 0)
				{
					tp_log_info("%s(line %d):synaptics_rmi4_f51_init:%d\n",__func__,__LINE__,retval);
					return retval;
				}
				break;
			}
...
flash_prog_mode:
	...
	tp_log_info("%s(line %d):Number of interrupt registers = %d\n",__func__,__LINE__,rmi4_data->num_of_intr_regs);
	...
	if (rmi->manufacturer_id != 1) {
		tp_log_err("%s(line %d):Non-Synaptics device found, manufacturer ID = %d\n",__func__,__LINE__,rmi->manufacturer_id);
	}
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			...
			tp_log_info("%s(line %d):Interrupt enable mask %d = 0x%02x\n",__func__,__LINE__,ii, rmi4_data->intr_mask[ii]);
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	tp_log_info("%s(line %d):in synaptics_rmi4_set_params\n",__func__,__LINE__);
	rmi = &(rmi4_data->rmi4_mod_info);
	...
	tp_log_info("%s(line %d):exit synaptics_rmi4_set_params\n",__func__,__LINE__);
	return;
}
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	int temp;
	int i =0;
	tp_log_info("%s(line %d):in synaptics_rmi4_set_input_dev\n",__func__,__LINE__);
	rmi4_data->input_dev = input_allocate_device();
	if (rmi4_data->input_dev == NULL) {
		tp_log_err("%s(line %d):Failed to allocate input device\n",__func__,__LINE__);
		retval = -ENOMEM;
	...
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to query device\n",__func__,__LINE__);
	...
	if (retval) {
		tp_log_err("%s(line %d):Failed to register input device\n",__func__,__LINE__);
		goto err_register_input;
	}

	tp_log_info("%s(line %d):exit synaptics_rmi4_set_input_dev\n",__func__,__LINE__);
	return 0;
...
err_input_device:
	tp_log_info("%s(line %d):exit synaptics_rmi4_set_input_dev : error %d\n",__func__,__LINE__,retval);
	return retval;
}
...
{
	unsigned char ii;
	tp_log_debug("%s(line %d):in synaptics_rmi4_free_fingers\n",__func__,__LINE__);
#ifdef TYPE_B_PROTOCOL
...
static int synaptics_rmi4_reinit_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_device_info *rmi;
	tp_log_debug("%s(line %d):in synaptics_rmi4_reinit_device\n",__func__,__LINE__);
	rmi = &(rmi4_data->rmi4_mod_info);
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			tp_log_debug("%s(line %d):Interrupt enable mask %d = 0x%02x\n",
				__func__,__LINE__, ii, rmi4_data->intr_mask[ii]);
			intr_addr = rmi4_data->f01_ctrl_base_addr + 1 + ii;
			retval = synaptics_rmi4_i2c_write(rmi4_data,
					intr_addr,
...
exit:
	...
	tp_log_debug("%s(line %d):exit synaptics_rmi4_reinit_device\n",__func__,__LINE__);
	return retval;
}
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	unsigned char command = 0x01;
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	tp_log_info("%s(line %d):in synaptics_rmi4_reset_device\n",__func__,__LINE__);
	mutex_lock(&(rmi4_data->rmi4_reset_mutex));
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to issue reset command, error = %d\n",__func__,__LINE__,retval);
		mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
		return retval;
	}
	...
	mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
	tp_log_info("%s(line %d):exit synaptics_rmi4_reset_device\n",__func__,__LINE__);
	return 0;
}
...
static void synaptics_rmi4_exp_fn_work(struct work_struct *work)
{
	...
	tp_log_info("%s(line %d):in synaptics_rmi4_exp_fn_work\n",__func__,__LINE__);
	mutex_lock(&exp_data.mutex);
	if (!list_empty(&exp_data.list)) {
	...
	mutex_unlock(&exp_data.mutex);
	tp_log_info("%s(line %d):exit synaptics_rmi4_exp_fn_work\n",__func__,__LINE__);
	return;
}
...
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	tp_log_info("%s(line %d):in synaptics_rmi4_new_function\n",__func__,__LINE__);
	...
	if (insert) {
		...
		if (!exp_fhandler) {
			tp_log_err("%s(line %d):Failed to alloc mem for expansion function\n",__func__,__LINE__);
			goto exit;
		}
...
exit:
	...
	tp_log_info("%s(line %d):exit synaptics_rmi4_new_function\n",__func__,__LINE__);
	return;
}
EXPORT_SYMBOL(synaptics_rmi4_new_function);

static ssize_t synaptics_virtualkeys_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
		__stringify(EV_KEY) ":"
		__stringify(KEY_BACK) ":142:1342:150:100"
		":" __stringify(EV_KEY) ":"
		__stringify(KEY_HOMEPAGE) ":360:1342:150:100"
		":" __stringify(EV_KEY) ":"
		__stringify(KEY_MENU) ":578:1342:150:100"
		"\n");
}
...
static struct kobj_attribute synaptics_virtualkeys_attr = {
	...
	.show = &synaptics_virtualkeys_show,
};
		...
		const struct i2c_device_id *dev_id)
{
	...
	struct kobject *properties_easywakeup_kobj;
	unsigned char device_ctrl;
	 
	if(cyttsp_device_check_ok)
	{
		printk("synaptics_rmi4_init cyttsp_device_check_ok=%d\n",cyttsp_device_check_ok);
		return -1;
	}
	printk("synaptics_rmi4_init cyttsp_device_check_ok=%d\n",cyttsp_device_check_ok);
	if (!i2c_check_functionality(client->adapter,
			I2C_FUNC_SMBUS_BYTE_DATA)) {
		tp_log_err("%s(line %d): SMBus byte data not supported\n",
				__func__,__LINE__);
		return -EIO;
	}

	tp_log_info("%s(line %d)begin to power on and reset touch controller\n",__func__,__LINE__);
	// gpio setting
	mt_set_gpio_mode(GPIO_CTP_EINT_PIN, GPIO_CTP_EINT_PIN_M_EINT);
	mt_set_gpio_dir(GPIO_CTP_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_CTP_EINT_PIN, GPIO_PULL_ENABLE);
	...
	if (!rmi4_data) {
		tp_log_err("%s(line %d): Failed to alloc mem for rmi4_data\n",
				__func__,__LINE__);
		return -ENOMEM;
	}
	...
	retval = sysfs_create_file(properties_kobj, &easy_wakeup_gesture.attr);
	if (retval)
	{
		kobject_put(properties_kobj);
		tp_log_err("%s: easy_wakeup_gesture create file error = %d\n", __func__,retval);
		return -ENODEV;
	}

	/*add the node synaptics_mmi_test apk to write*/
	retval = sysfs_create_file(properties_kobj, &easy_wakeup_position.attr);
	if (retval)
	{
		kobject_put(properties_kobj);
	...
	retval = sysfs_create_file(properties_kobj, &easy_wakeup_control.attr);
	if (retval)
	{
		kobject_put(properties_kobj);
		tp_log_err("%s: easy_wakeup_control create file error\n", __func__);
		return -ENODEV;
	}
#endif 
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			&device_ctrl,
			sizeof(device_ctrl));
	rmi4_data->easy_wakeup_fastrate = device_ctrl;
	if (retval < 0) {
		tp_log_err("%s: Failed to get tpd  info\n",__func__);
		return;
	}
#endif 
#ifdef CONFIG_HAS_EARLYSUSPEND
	...
	if ( IS_ERR(thread) ) {
		...
		tp_log_err("%s(line %d): failed to create kernel thread: %d\n",__func__,__LINE__, retval);
	}

	retval = synaptics_rmi4_irq_enable(rmi4_data, true);
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to enable attention interrupt\n", __func__,__LINE__);
		goto err_enable_irq;
	}
	...
	if (!exp_data.initialized) {
		...
		INIT_LIST_HEAD(&exp_data.list);
		exp_data.initialized = true;
		tp_log_info("%s(line %d):set exp_data.initialized = true\n",__func__,__LINE__);
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) 
	{
		...
		if (retval < 0) {
			tp_log_err("%s(line %d:Failed to create sysfs attributes\n",__func__,__LINE__);
			goto err_sysfs;
		}
	}
...
err_set_input_dev:
	kfree(rmi4_data);
	tp_log_info("%s(line %d):exit from synaptics_rmi4_probe:%d\n",__func__,__LINE__,retval);
	return retval;
}
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to enter sleep mode\n",__func__,__LINE__);
		rmi4_data->sensor_sleep = false;
		return;
	}
...
static void synaptics_rmi4_sensor_wake(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s(line %d):Failed to wake from sleep mode\n",__func__,__LINE__);
		rmi4_data->sensor_sleep = true;
		return;
	}
...
static void synaptics_put_device_into_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s: Failed to enter sleep mode1\n",__func__);
		rmi4_data->sensor_sleep = false;
		return;
	}
	...
	if (retval < 0) {
		tp_log_info("%s: Failed to enter sleep mode2\n",__func__);
		rmi4_data->sensor_sleep = false;
		return;
	} else{
		rmi4_data->sensor_sleep = true;
	...
	if (retval < 0) {
		tp_log_info("%s: Failed to set fastrate\n",__func__);
		rmi4_data->sensor_sleep = false;
		return;
	} else{
		rmi4_data->sensor_sleep = true;
    ...
    tp_log_info(" read from  f11_ctrl_base_addr92  rmi4_data->f11_ctrl_base_addr  = 0x%0x rmi4_data->f11_ctrl_base_addr+92 =0x%0x\n",rmi4_data->f11_ctrl_base_addr,
    ...
    retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s: Failed to set wakeup mode\n",__func__);
		rmi4_data->sensor_sleep = false;
		return;
	}else {
		rmi4_data->sensor_sleep = true;
    ...
    tp_log_info(" read from  f11_ctrl_base_addr92  ctrl1 = %0x retval =%0x\n",device_ctrl,retval);
	device_ctrl = 0x03;
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			rmi4_data->f11_ctrl_base_addr+45,
			&device_ctrl,
...
static void synaptics_put_device_outof_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	unsigned char device_ctrl;
	tp_log_info("synaptics_put_device_outof_easy_wakeup  rmi4_data->sleep_gesture_flag =%d\n",rmi4_data->sleep_gesture_flag);
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to out of  sleep mode\n",
				__func__);
		rmi4_data->sensor_sleep = true;
		return;
	}
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s: Failed to out of  sleep mode1\n",
				__func__);
		rmi4_data->sensor_sleep = true;
		return;
	} else {
...
{
	tp_log_info("synaptics_put_device_into_sleep easy_wakeup_gesture=%0x\n",rmi4_data->easy_wakeup_gesture);
	if (  rmi4_data->easy_wakeup_gesture != 0 ){		
		synaptics_put_device_into_easy_wakeup(rmi4_data);
	}
	else{
	...
	tp_log_info("exit synaptics_put_device_into_sleep\n");
	return;
}

static void synaptics_put_device_into_wake(struct synaptics_rmi4_data *rmi4_data)
{
	tp_log_info("synaptics_put_device_into_wake easy_wakeup_gesture=%0x\n",rmi4_data->easy_wakeup_gesture);
	if (rmi4_data->easy_wakeup_gesture != 0 && (rmi4_data->touch_stopped != true) ) {	
		synaptics_put_device_outof_easy_wakeup(rmi4_data);
	}
	else{
		synaptics_rmi4_sensor_wake(rmi4_data);
		synaptics_rmi4_irq_enable(rmi4_data, true);//sunlibin 11-20
	}
	tp_log_info("exit synaptics_put_device_into_wake\n");
	return;
}
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	if (rmi4_data->stay_awake) {
		tp_log_debug("%s: stay_awake = true \n",__func__);
...
{
	tp_log_info("synaptics I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);

	if(i2c_add_driver(&tpd_i2c_driver)!=0)
	{
		tp_log_err("%s%d: Error unable to add i2c driver.\n",__FUNCTION__,__LINE__);
		return -1;
	}
#ifdef TPD_HAVE_BUTTON     
...
static int __init synaptics_rmi4_init(void)
{
	...
	if(tpd_driver_add(&synaptics_rmi4_driver) < 0){
		tp_log_err("%s%d:Fail to add tpd driver\n",__FUNCTION__,__LINE__);
		return -1;
	}
